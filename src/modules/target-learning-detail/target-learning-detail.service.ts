...
export class TargetLearningDetailService {
    ...
    async getAllUnitProgress(targetLearningId: string): Promise<any[]> {
        ...
        if (targetLearningDetails.length === 0) {
            throw new NotFoundException(`No TargetLearningDetails found for TargetLearning ID ${targetLearningId}`);
        }
        ...
        const response = targetLearningDetails.map((detail) => ({
            ...
            section: detail.section ? { id: detail.section.id, name: detail.section.name } : null,
            level: detail.level ? { id: detail.level.id, name: detail.level.name } : null,
            unitProgresses: unitProgresses
                .filter((unitProgress) => unitProgress.targetLearningDetail.id === detail.id)
                .map((unitProgress) => ({
                    unitId: unitProgress.unit.id,
                    unitTitle: unitProgress.unit.title,
                    level: unitProgress.unit.level
                        ? { id: unitProgress.unit.level.id, name: unitProgress.unit.level.name }
                        : null,
                    domain: unitProgress.unit.domain
                        ? { id: unitProgress.unit.domain.id, content: unitProgress.unit.domain.content
                         }
                        : null,
