  ...
  export class BaseController<T> {
    ...
    async create(@Body() createDto: any) {
      try {
        ...
        return ResponseHelper.error(error, HttpStatus.BAD_REQUEST, `Failed to create ${this.entityName}`);
      }
    }
    ...
    async findAll(@Query() paginationOptions: PaginationOptionsDto) {
      try {
        ...
        return ResponseHelper.error(error, HttpStatus.INTERNAL_SERVER_ERROR, `Failed to retrieve ${this.entityName} list`);
      }
    }
    ...
    async findOne(@Param('id') id: string) {
      try {
        ...
        return ResponseHelper.error(error, HttpStatus.INTERNAL_SERVER_ERROR, `Failed to retrieve ${this.entityName}`);
      }
    }
    ...
    async update(@Param('id') id: string, @Body() updateDto: any) {
      try {
        ...
        return ResponseHelper.error(error, HttpStatus.BAD_REQUEST, `Failed to update ${this.entityName}`);
      }
    }
    ...
    async remove(@Param('id') id: string) {
      try {
        ...
        return ResponseHelper.error(error, HttpStatus.INTERNAL_SERVER_ERROR, `Failed to delete ${this.entityName}`);
      }
    }
  }
